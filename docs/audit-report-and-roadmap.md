...
| 26/09/2026 | RBAC toàn hệ thống | **Mọi thao tác là một permission**; ngoại lệ duy nhất: **Super Admin** (luôn toàn quyền, bất biến) và **luật quan hệ** (GV chính xác nhận báo cáo trực lớp, người lập phiếu không tự duyệt, không tự chấm KPI, người được giao việc, người tham gia ticket, học viên xem dữ liệu của mình, GV của lớp). **Phạm vi dữ liệu** Của tôi / Chi nhánh / Toàn hệ thống cấu hình theo module (`<module>.scope_*`). Admin: ma trận quyền theo module cho vai trò + ghi đè theo người; tạo / nhân bản / xóa vai trò; không tự khóa mình; mọi thay đổi ghi nhật ký; seed chỉ thêm (không ghi đè cấu hình Admin khi deploy). Mặc định giữ hành vi hiện tại + các quyết định BA 26/09. Chi tiết: `docs/rbac.md` |
...
| 4 | Thu học phí, hỗ trợ, nghiệm thu | ✅ | 100% (phạm vi đã chốt) | Xong: học phí (phiếu thu nháp → gửi duyệt → duyệt/trả về, dải số hóa đơn theo chi nhánh, hủy HĐ, bảo lưu/khất nợ, quá hạn ≥7 / 1–6 ngày, nhắc nợ theo mốc), hoàn phí theo BA (1 tuần + cùng tháng, ưu tiên chuyển nhượng, ảnh bằng chứng, cờ quá hạn), SePay chống ghi nhận 2 lần + QR theo chi nhánh, báo cáo doanh thu/khoản chi theo phạm vi; nhật ký trước/sau + hoàn tác, phân quyền cá nhân, quyền kế toán do Admin phân (26/09), Học vụ full CRM trừ xóa; giao việc 2 chiều, trợ giảng 3 ca, trực lớp Q8; ticket; dashboard vai trò; đối chiếu mockup; dữ liệu demo + nghiệm thu + test đi qua BPMN 1–22 (829 test pass). **Sau Phase 4:** ✅ RBAC toàn hệ thống — bỏ phân quyền cứng theo vai trò, Admin cấu hình quyền + phạm vi dữ liệu theo vai trò / người (`docs/rbac.md`). **Chờ:** Zalo ZNS/SMS thật, key SePay thật, UAT với người dùng |
...
**Quyết định phát sinh:** Q8 tạm: báo cáo trực lớp không ảnh do GV chính của lớp duyệt (lớp chưa có GV chính → người có quyền duyệt công việc cùng chi nhánh). ✅ Thay bằng luật A6 Q8 (25/09) ở `feat/phase4-platform-parity`: chỉ GV chính, lớp chưa có GV chính → **người giao việc**; Học vụ / Quản lý / Admin không còn duyệt thay. Ticket chưa phân công báo cho người có `support_ticket.assign` cùng chi nhánh người tạo; không có ai → Admin.
...
#### Nghiệm thu Phase 3 — Buổi dạy → chấm công → phạt / hoa hồng / KPI → bảng lương → duyệt → nhân viên xem lương (nhánh `feat/phase3-seed-acceptance`)
**Kịch bản đã kiểm thử** (`tests/Feature/Phase3AcceptanceTest.php`, một luồng liền mạch qua HTTP bằng đúng vai trò, theo A6 Q3 + "Hoa hồng tuyển sinh (bản sửa)"; kỳ 8/2026 tính 01/09 duyệt 02/09, kỳ 9 tính + duyệt 01/10, kỳ 10 tính 01/11):
- [x] Quản lý cơ sở thêm **đơn giá buổi riêng** có 2 phiên bản (200.000đ/buổi từ 01/07, 250.000đ/buổi từ 01/09); Kế toán không có quyền (403).
- [x] Sale nhập 2 khách → Học vụ chuyển "Đang tư vấn" (Sale bị 403) → Sale **Chốt & Xếp lớp** đã đóng phí → Kế toán duyệt phiếu thu (Sale 403).
- [x] **Check-in chỉ cho buổi thật hôm nay được phân công**: buổi chưa tới / buổi của GV khác bị từ chối, lớp không dạy 403; check-in đúng buổi → chờ duyệt, 1,5 giờ; **bấm lại không tạo thêm, không ghi đè giờ** (sửa trong nhánh này).
- [x] **Chấm công tay** (Học vụ; GV 403): thiếu lý do / giờ ra trước giờ vào / **ngày chưa diễn ra** bị chặn; ca hợp lệ tự gắn buổi học, lưu giờ vào/ra; **nhập lần 2 cùng buổi bị chặn**, GV không check-in chồng lên ca chấm tay. Ca workshop không có buổi trên lịch → Học vụ **từ chối (bắt buộc lý do)**; GV không tự duyệt ca (403).
- [x] **Kỷ luật**: Học vụ ghi nhận (không cần số tiền; GV không lập được biên bản — 403); chỉ người vi phạm thấy / giải trình (người khác 403, nội dung quá ngắn bị chặn); **chốt theo loại lỗi** — lỗi vận hành: Học thuật 403, Quản lý quyết phạt (thiếu số tiền bị chặn) → hạn nộp 2 ngày; lỗi chuyên môn: Học vụ 403, Học thuật quyết phạt → Quản lý ghi nhận **nộp trực tiếp** (không trừ lương). Biên bản quá hạn → **trừ lương** kỳ 8 → "Đã trừ" khi duyệt; biên bản vi phạm 28/08 quyết phạt 31/08 (hạn 02/09) → **không** trừ kỳ 8, trừ kỳ 9.
- [x] **KPI Học vụ** 15 mục / 6 nhóm: Học vụ **tự chấm mình 403**; Quản lý chấm (mục 1.2 đạt 50%) → 92,5% → KPI = 2.000.000 × 92,5% = **1.850.000đ**.
- [x] **Kỳ 8**: Quản lý không tạo / tính kỳ (403), GV không vào danh sách bảng lương (403); Kế toán tạo kỳ (trùng tháng bị chặn) → chỉ 4 người có phiếu. Kế toán nhập **bậc KPI giữ HS** (bậc 30.000 bị chặn; Quản lý 403), **TNCN** (Part-time bị bỏ qua), **KPI tự do**, **phụ cấp tự do có tên** → "Đồng bộ & Tính lại" giữ nguyên các khoản nhập tay. Số liệu:
  - GV Part-time: 3 buổi (2 check-in + 1 chấm tay; workshop bị từ chối) × 200.000 + KPI 4/5 HS giữ được × 20.000 + Gửi xe 100.000 − phạt quá hạn 100.000 = **680.000đ**, không BHXH / Công đoàn / TNCN.
  - GV Full-time: 10.000.000 + KPI 1.000.000 + phụ cấp 500.000 − BHXH 10,5% 1.050.000 − Công đoàn 0,5% 50.000 − TNCN 250.000 = **10.150.000đ** (2 buổi dạy chỉ để đối soát, phạt đã nộp không trừ).
  - Học vụ: 8.000.000 + 1.850.000 − 840.000 − 40.000 = **8.970.000đ**. Sale: 7.000.000 − 735.000 − 35.000 = **6.230.000đ**, 2 HS chốt → bậc 3%, hoa hồng **hoãn 540.000đ** (chưa đủ 30 ngày).
- [x] **Chỉ Admin duyệt** (Kế toán, Quản lý 403). Kỳ đã duyệt **khóa**: chấm công tay / duyệt ca / lập biên bản ngày trong kỳ bị từ chối, **đánh giá KPI tháng đã khóa bị từ chối** (sửa trong nhánh này), nhập tay trên phiếu / tính lại / ghi nhận nộp phạt đã trừ → 422.
- [x] **"Lương của tôi"**: trước khi duyệt không có phiếu (và GV không mở được phiếu lương quản trị — 403); sau khi duyệt thấy đúng phiếu 680.000đ với dòng "KPI giữ học sinh"; người không có phiếu không thấy gì; kỳ chưa duyệt → 404.
- [x] **Kỳ 9**: đơn giá phiên bản mới 250.000đ/buổi; Sale / Quản lý tick mốc chăm sóc (khách 1 đủ 3/3, khách 2 2/3) → **khách 1 trả 270.000đ, khách 2 tiếp tục hoãn** ("2/3 mốc"). GV **nộp phạt trực tiếp sau lần tính** → Admin duyệt bị chặn "phải tính lại" (sửa trong nhánh này) → tính lại bỏ khoản trừ → duyệt; khoản hoa hồng đã trả được tất toán.
- [x] **Kỳ 10**: khách 2 đủ 3/3 → **hoa hồng hoãn được trả ở kỳ sau** (270.000đ), khoản đã trả không trả lại; Sale xem được kỳ 9, kỳ 10 (chưa duyệt) 404.

**Lỗi phát hiện và đã sửa (không sửa view):**
- [x] **Sửa đánh giá KPI của tháng đã khóa lương**: `KpiController::evaluateStore` không kiểm tra kỳ lương → sau khi Admin duyệt vẫn đổi được điểm KPI tháng đó (dữ liệu KPI lệch với phiếu lương đã duyệt). Nay từ chối khi kỳ lương tháng đó đã duyệt / đã chi trả.
- [x] **Trừ phạt 2 lần**: biên bản vi phạm kỳ trước nhưng quá hạn trừ vào kỳ này; nếu sau lần tính nhân sự nộp trực tiếp (hoặc được miễn), biên bản bị gỡ khỏi phiếu nhưng `PayrollPeriod::hasChangesSinceCalculation` không nhận ra (chỉ xét biên bản có ngày vi phạm trong kỳ / còn gắn phiếu) → Admin duyệt được phiếu vẫn trừ tiền phạt đã nộp. Nay so tiền phạt trên phiếu với các biên bản còn gắn phiếu (`penaltyDeductionOutOfSync`) → bắt buộc tính lại.
- [x] **Check-in lại ghi đè ca đã duyệt**: bấm check-in lần 2 cùng buổi (`TeacherPortalController::checkin`, `updateOrCreate`) đưa ca đã duyệt về "chờ duyệt" và đổi giờ check-in → kỳ lương "cần tính lại". Nay báo "đã check-in lúc …"; chỉ ca bị từ chối được check-in lại.
- [x] **Chấm công tay cho ngày tương lai**: `storeTimesheet` nhận ngày chưa diễn ra (tính công trước cho buổi chưa dạy). Nay `before_or_equal:today`.
- [x] Seeder: `MasterEntitySeeder` chạy lại `db:seed` **ghi đè kỳ PR-2026-08 về "draft" kèm tổng tiền giả** (mở khóa kỳ đã duyệt) và ghi đè phiếu lương đã tính → nay `firstOrCreate`, không tạo phiếu mẫu khi kỳ đã khóa; ca chấm công mẫu không gắn buổi học để "chờ duyệt" thay vì "hợp lệ". `WorkTaskSeeder` nhân bản `hr_daily_demands` mỗi lần seed trên SQLite (so ngày với datetime).

**Đối chiếu A3 "Chấm công, Phạt, Lương" + B4 "Lương & chấm công" + Phần C Phase 3:**

| Mục | Kết quả | Bằng chứng / ghi chú |
|---|---|---|
| A3 · Tính lại lương không xóa bản ghi cũ | ✅ Xong | `runCalculation` dùng lại phiếu cũ, xóa phiếu không còn đủ điều kiện, gỡ / gắn lại phạt, thu hồi, hoa hồng; test tính lại giữ số |
| A3 · Chưa tính thưởng tái tục | ✅ Xong (bảng % chờ BA) | `PayrollFormulaService::renewalBonusFor`; demo `gv.cohuu2` kỳ C 1% × doanh thu lớp |
| A3 · Kế toán / Quản lý duyệt lương | ✅ Xong | `payroll.approve` chỉ Admin; test Kế toán + Quản lý 403 |
| A3 · Quy trình phạt (giải trình, HT/CM chốt theo loại lỗi, nộp 2 ngày) | ✅ Xong | `PenaltyController` + `Penalty::CATEGORIES`, `deductibleFor`; test đủ nhánh. **Sửa thêm:** trừ 2 lần khi nộp sau lần tính |
| A3 · Chấm công tay không bắt buộc lý do; chấm trùng tính 2 lần | ✅ Xong + sửa thêm | `storeTimesheet` + `findDuplicate`. **Sửa thêm:** check-in lại ghi đè ca đã duyệt; chấm tay ngày tương lai |
| A3 · "Lương của tôi" hiện bảng chưa duyệt; tự chấm KPI | ✅ Xong | `mySalary` chỉ `LOCKED_STATUSES`; `evaluateStore` chặn tự chấm; test |
| B4 · Màn phiếu lương từng người, sửa từng khoản | ✅ Xong | `payroll.records.show` / `adjust`; test |
| B4 · 3 màn chi tiết lương sai cột / tổng không khớp | ✅ Xong (logic) | Cột theo Q3 (`feat/phase3-formula`); giao diện do nhánh view đang làm |
| B4 · Đơn giá GV / mốc hoa hồng không theo người, không hiệu lực / lịch sử | ✅ Xong | `teacher_hourly_rates` (đ/buổi, ngày hiệu lực), `commission_tiers` phiên bản; test + demo |
| B4 · Chấm công tay thiếu giờ vào/ra, lý do, chặn kỳ khóa | ✅ Xong | test (thiếu lý do, giờ ra trước giờ vào, kỳ khóa) |
| B4 · Lịch sử đồng bộ luôn trống / ghi cứng "thành công" | ✅ Xong (trung thực) | Trạng thái trống thật; chưa có máy chấm công → xem "Còn tồn" |
| B4 · Lương của tôi: chọn kỳ, số khớp, dấu "đã xác thực" | ✅ Xong (logic) | `period_id`, cùng `earningLines/deductionLines`; view do nhánh giao diện |
| B4 · Danh sách phạt: tìm kiếm, lọc bước, phân trang, không bắt nhập tiền | ✅ Xong | `PenaltyController::index`, `storePenalty` amount nullable |
| C3.1 Chấm công: chỉ buổi có thật; chấm tay bắt buộc lý do, không trùng | ✅ Xong + sửa thêm | Như trên; test + demo |
| C3.2 Kỷ luật đủ bước, quá hạn trừ lương | ✅ Xong + sửa thêm | Như trên |
| C3.3 Hoa hồng: khách mới, tiền thực thu gồm giáo trình; hoàn phí chọn thu hồi (gợi ý < 1 tháng), chuyển nhượng không thu hồi | ✅ Xong | `SalesCommissionService`; bậc 3/4/5% + gate kép; test + demo (thu hồi 90.000đ) |
| C3.4 Tính đủ khoản; tính lại đúng; kỳ duyệt khóa toàn bộ; chỉ GĐ duyệt; my-salary; không tự chấm KPI | ✅ Xong + sửa thêm | **Sửa thêm:** khóa đánh giá KPI tháng đã duyệt |
| Kết quả: lương khớp bảng Excel đang dùng | ⚠️ Chưa làm | Chưa có file Excel của Kế toán |
| Kết quả: chạy trọn luồng buổi dạy → … → nhân viên xem lương | ✅ Xong | `Phase3AcceptanceTest` + `DemoPhase3Seeder` |
| Mockup các màn Phase 3 | 🟦 Nhánh khác đang làm | Nhánh này không sửa view |

**Dữ liệu demo:** `Database\Seeders\DemoPhase3Seeder` (`DatabaseSeeder` gọi sau `DemoPhase2Seeder`, cùng điều kiện môi trường). Các sự kiện chạy theo thời gian thật trong quá khứ (check-in lúc vào ca, tính kỳ tháng trước ngày 1, duyệt ngày 2…), qua controller / service thật (TeacherPortalController@checkin, PayrollController chấm tay / duyệt ca / đơn giá / mốc hoa hồng / tham số / tạo – tính – nhập tay – duyệt kỳ, PenaltyController, KpiController, CrmController thêm khách / Chốt & Xếp lớp / tick mốc chăm sóc, TuitionController phiếu thu / hoàn phí có thu hồi). Idempotent, ~2,5 giây, trong 1 transaction. Sau `migrate:fresh --seed` trên SQLite trắng (ngày chạy 25/09/2026):
- 5 đơn giá buổi riêng (GV part-time 2 phiên bản); 74 ca chấm công (67 check-in + 2 chấm tay + 5 từ seeder khác; 72 hợp lệ, 2 bị từ chối — workshop + ca mẫu không có buổi); 1 lần chấm tay trùng bị chặn.
- 9 biên bản: chờ giải trình 1, đã giải trình 1, xác nhận lỗi 2 (1 của MasterEntitySeeder), quyết phạt 2, đã nộp 1, đã trừ lương 1, đã hủy 1.
- 4 đánh giá KPI Học vụ (2 người × 2 tháng); mốc hoa hồng 3% / 4% / 5%; 6 khoản hoa hồng (đã trả 1, trả trong kỳ C 2, hoãn 3); 1 hoàn phí có thu hồi (−90.000đ).
- PR-2026-08 **Đã duyệt**, 10 phiếu, tổng thực lĩnh **70.212.600đ**; PR-2026-09 **Đang soát**, 10 phiếu, tổng **76.067.600đ**. Thực lĩnh từng người (kỳ 8 / kỳ 9): Học thuật 14.100.000 / 14.100.000; Học vụ CG 9.822.600 / 9.910.000; Học vụ BD 9.015.000 / 9.177.600; Sale CG 6.700.000 (hoa hồng 270.000, hoãn 810.000) / 6.880.000 (hoa hồng 540.000, hoãn 540.000, thu hồi 90.000); Sale BD 6.230.000 / 6.230.000 (hoãn 270.000); GV full-time CG 11.680.000 / 11.680.000; GV full-time BD 10.890.000 / 10.980.000 (tái tục 90.000); GV part-time CG 900.000 (4 buổi × 220.000) / 3.880.000 (15 buổi × 250.000, KPI 9 HS × 20.000, phạt 150.000); TA CG 495.000 / 1.695.000; TA BD 380.000 / 1.535.000.

Test: thêm `Phase3AcceptanceTest` (1 luồng, 196 assertion) và `tests/Feature/DemoPhase3SeederTest.php` (đủ trạng thái, chạy lại không đổi số dòng / tổng tiền, kỳ đã duyệt không bị mở, mở được các màn bằng tài khoản demo). Hướng dẫn + tài khoản: `README.md`. Toàn bộ bộ test xanh: **770 test, 8877 assertion** (không sửa test cũ).

**Còn tồn / cần BA xác nhận:**
- [ ] **Lương buổi có GVNN** (Part-time): vẫn là dòng nhập tay "chờ BA chốt".
- [ ] **Bảng % thưởng tái tục đầy đủ** (mới có 0 nghỉ → 1%, 1 nghỉ → 0,7%; 2, 3 và vượt bảng = 0% chờ BA).
...
**Triển khai:** không có migration. Demo/staging: `php artisan migrate:fresh --seed` hoặc `php artisan db:seed --class=DemoPhase3Seeder` (sau Phase 1–2). Production **không** đặt `SEED_DEMO=true`.
...
**Còn tồn / cần BA xác nhận:**
- [x] **Luật hoàn phí A6** và **Q8** (sau khi gộp 3 nhánh Phase 4, 26/09/2026 — nhánh `feat/ba-permissions`, commit riêng): `Phase4AcceptanceTest` nay kiểm hoàn tiền thiếu "lý do không chuyển nhượng" bị chặn, chuyển nhượng do Quản lý duyệt, hoàn tiền Quản lý / Kế toán 403, Admin thiếu ảnh bằng chứng → lỗi, có ảnh → duyệt (file lưu riêng tư), hạn = lập + 7 ngày, hồ sơ quá hạn hiện cờ "Quá hạn xử lý" và vẫn duyệt được (giữ cờ); hủy HĐ Quản lý / Kế toán 403, Admin duyệt; Q8 "Gắn lớp" bắt buộc buổi học, lớp chưa có GV chính → người giao việc xác nhận, không ảnh + không ai xác nhận → chặn. `DemoPhase4Seeder` theo luật mới: mỗi kế toán chi nhánh tự cấp dải số / TK của chi nhánh mình, hồ sơ hoàn phí E có lý do không chuyển nhượng, hủy HĐ khách J do Admin duyệt, giao 3 ca TA kèm buổi học (buổi thật trong ngày, không có thì tên buổi).
...
#### Phase 4 — Đối chiếu mockup nhóm nền tảng + luật Q8 báo cáo trực lớp (nhánh `feat/phase4-platform-parity`)
**Phương pháp:** mở `code.html` của mockup (`ui-full-tinh-nang-menglish/epic-5/quan-ly-tai-khoan-vai-tro`, `phan-quyen-chi-tiet-ca-nhan`, `quan-ly-danh-muc-he-thong`, `nhat-ky-van-hanh`, `phan-cong-cong-viec/*`) cạnh view Blade; sửa hết P1 (thiếu trường / cột / lọc / nút / thao tác, dữ liệu giả) rồi P2 (nhãn, icon, bố cục, token màu / chữ, `x-ui.*`). Phần tử mockup trái A6 thì theo A6.

**Luật A6 Q8 (25/09/2026) — Báo cáo trực lớp — đã căn đúng:**
- [x] Ảnh **không bắt buộc**; nộp được **nhiều ảnh** (`class_reports.board_images`, tối đa 10, `SafeUploadService`).
- [x] Có **≥ 1 ảnh** → báo cáo "Đã xác nhận", đầu việc **"Trực lớp"** tự **Hoàn thành** (không cần ai duyệt, không gửi thông báo duyệt).
- [x] Không ảnh → báo cáo + đầu việc **"Chờ xác nhận"**; người xác nhận = **GV chính của lớp** (`classes.teacher_id`); lớp **chưa có GV chính** (hoặc GV chính chính là người nộp) → **người giao việc** (người tạo đầu việc "Trực lớp"). Tính lại theo GV chính hiện tại khi duyệt (`ClassReport::currentConfirmerId()`), lưu `confirmer_id` lúc nộp.
- [x] **Chỉ đúng người xác nhận** được Xác nhận / Trả về (cả qua màn "Xác nhận hoàn thành thủ công" và nút duyệt đầu việc `tasks.approve/reject`); Học vụ / Quản lý cơ sở / Admin / người nộp → 403 (trước đây Học vụ / Quản lý duyệt thay được — sai A6).
- [x] **Thông báo đúng 1 người**: người xác nhận nhận `class_report_pending` (không còn gửi Học vụ / Quản lý / Admin); xác nhận / trả về báo lại người nộp; trả về → đầu việc quay "Đang thực hiện" kèm lý do, nộp lại được.
- [x] Báo cáo tự gắn đầu việc "Trực lớp" đang mở của người nộp cho lớp trong ngày (nếu không mở từ Portal). Không ảnh + lớp chưa có GV chính + không có đầu việc được giao → **chặn nộp** kèm hướng dẫn (không có ai để xác nhận).
- [x] Việc thường (không phải báo cáo trực lớp) gửi "Chờ xác nhận" không ảnh → báo **người giao việc**.

| Màn | P1 đã sửa | P2 đã sửa | Chưa làm / bỏ theo A6 (lý do) |
|---|---|---|---|
| **Tài khoản & vai trò** (danh sách / chi tiết / form) | Hồ sơ nhanh (drawer) có **kiêm nhiệm thật**: vai trò phụ + lớp đang phụ trách (bỏ dòng tĩnh "Chưa có thông tin phân công kiêm nhiệm"), **trạng thái hợp đồng tính theo ngày kết thúc** (trước luôn "Đang hiệu lực" khi có loại HĐ), link tải file HĐ (riêng tư); badge vai trò chính / "(kiêm nhiệm)"; lọc **trạng thái** (hoạt động / vô hiệu hóa / HĐ sắp-đã hết hạn); "Khối học thuật" đếm đủ GV FT/PT, Học vụ, Học thuật, TA; form có **"Vai trò kiêm nhiệm"** (chỉ vai trò người thao tác được gán, chặn vượt phân cấp); chi tiết hiện vai trò chính + kiêm nhiệm | Token / `x-ui.*`, nhãn vai trò ngắn (`AclHelper::shortRoleLabel`), hành động dòng theo mockup (phân quyền · khóa/mở · xem + menu), bỏ placeholder giả ("Nguyễn Văn A", "0912 345 678") | Mockup "Thêm nhân viên mới" dạng modal 5 trường → giữ **trang form đầy đủ** (có hồ sơ, hợp đồng, kiêm nhiệm). Tab "Nhân viên" duy nhất của mockup không làm (không có loại tài khoản khác) |
| **Phân quyền cá nhân** | Ma trận **checkbox** Xem / Thêm / Sửa / Xóa (ô tích = quyền hiệu lực sau phân quyền cá nhân; gửi inherit/allow/deny nên không nhân bản quyền vai trò thành override), "Cấp thêm" / "Thu hồi" hiển thị trên ô; **Chọn tất cả / Bỏ chọn / Đặt lại mặc định**; **nhấn đúp tiêu đề cột** áp cả cột; "Không có quyền truy cập" cho module không có quyền; chip phạm vi đã chọn; thẻ **Tổng số Module / Quyền truy cập (N thao tác) / Phạm vi dữ liệu (N đơn vị)**; "Xem nhật ký" lọc sẵn theo người | Breadcrumb "Nhân sự › Phân quyền cá nhân", banner bảo mật, icon module, token | Ô phạm vi dạng gõ tag (mockup "Gõ tên lớp/cơ sở...") → giữ **chọn nhiều** (dữ liệu thật). Phạm vi chi nhánh/lớp vẫn chỉ áp cho Lớp học (việc tồn Phase 4 nền tảng) |
| **Danh mục hệ thống** | **Panel "Thêm giá trị mới / Sửa giá trị" bên phải** trên trang danh sách (Mã, Tên, Thứ tự, trạng thái, "Lưu thông tin" / "Hủy bỏ", ghi chú thứ tự hiển thị); **mã gợi ý kế tiếp** (`SRC_NN`, `LOST_NN`, `POS_NN`, `FINE_NN`); thứ tự tự nối cuối; tìm kiếm mã/tên; **"Làm mới"**; form riêng không còn hiện mã kỹ thuật `lead_source`; `?type` sai → tab mặc định | Tiêu đề + mô tả mockup, tab, badge Đang dùng / Đã ngừng, nút `settings_backup_restore` "Kích hoạt lại", xác nhận trước khi ngừng dùng | — |
| **Nhật ký vận hành** | **Chip module** (Tất cả / CRM / Giáo trình / Lớp & Điểm danh / Phân quyền / Học phí / Lương / Công việc & Ticket / Cấu hình); tìm theo người thực hiện **hoặc mã bản ghi** (`#id`, batch); khoảng ngày có kiểm tra; cột **Người thực hiện (vai trò + #id) / Thời điểm / Module / Loại (Thêm/Sửa/Xóa) / Hành động** + nút xem; **panel "Chi tiết đối chiếu"**: Module, Thao tác bởi, **Dữ liệu trước / Dữ liệu sau**, **Chi tiết kỹ thuật** (IP, User-Agent, Transaction ID = batch), **Hoàn tác** (Admin; hiện lý do nếu không hoàn tác được), "Đóng chi tiết"; **"Xuất Excel" ra .xlsx thật** (CSV vẫn có `?format=csv`) | Tiêu đề "Nhật ký vận hành", "Làm mới", token, phân trang "nhật ký" | Việc tồn "xuất CSV chưa phải .xlsx" của Phase 4 nền tảng ✅ |
| **Danh sách công việc** | Modal "Thay đổi trạng thái" hiện **đúng trạng thái đích**, **bắt buộc lý do khi Bị chặn / Hủy** (kiểm tra cả server); modal "Giao việc" **mở lại kèm lỗi từng trường** khi validate sai; ô chọn người nhận hiện **nhãn vai trò** (trước là mã `academic_staff`); lọc **Loại** (Phát sinh / Lặp); **việc "Lặp đi lặp lại" hoàn thành → tự sinh lượt kế tiếp** theo tần suất (trước chỉ là nhãn); nút "Chờ xác nhận" kèm số | Tab theo thứ tự mockup Của tôi / Tôi giao / Tất cả, token, badge trạng thái theo token `status-*`, phân trang "công việc" | Chuông thông báo trong header màn (mockup) → dùng chuông chung của topbar |
| **Form giao việc** | Dùng chung trường với modal (partial `tasks/partials/task-form-fields`), nhãn vai trò, lỗi từng trường, tần suất có ghi chú tự sinh lượt | Token / `x-ui.*` | — |
| **Tạo lượt giao việc cho Trợ giảng** (TA 3 ca) | "Chọn Trợ giảng" **chỉ tài khoản trợ giảng** trong phạm vi (trước lẫn GV, Học vụ, email `ta.%`); lớp theo phạm vi người giao; **"Gắn lớp?" bắt buộc chọn Lớp + Buổi học** (ô chọn **buổi học thật** của lớp trong ngày giao, không có buổi → nhập tên); **hạn từng ca theo buổi học** (Trước giờ học = giờ vào lớp, Trong giờ học = giờ tan, Sau giờ học = tan + 60'); **gửi sau 15h30 → báo Admin** (mockup); kiểm tra chi nhánh của Quản lý | Tiêu đề / mô tả / nhãn theo mockup, nội dung dạng textarea, token | — |
| **Nhiệm vụ hôm nay (Portal TA)** | Trạng thái **"Quá hạn"** thật: lệnh `tasks:mark-overdue` (15 phút/lần) chuyển việc Mới / Đang làm qua hạn (ngày + giờ) → Quá hạn, báo người làm 1 lần; thẻ hiện **"Trễ N giờ"** + "Hoàn thành gấp", **lý do Bị chặn**, lý do bị trả về, "Đang chờ GV chính / người giao việc xác nhận báo cáo trực lớp"; khung **"Bằng chứng hình ảnh"** + lưu ý theo mockup; chặn hoàn thành việc đã xong / đã hủy / đang chờ; chọn TA ngoài phạm vi → 403 | "Nhiệm vụ hằng ngày", badge in hoa, token | — |
| **Nộp báo cáo trực lớp** | Luật Q8 ở trên; **chỉ lớp mình phụ trách** (trước liệt kê mọi lớp đang học) và **danh sách học sinh thật của lớp** (trước rơi về `Student::all()` khi lớp trống); ô **Buổi học** chọn buổi thật (Buổi N – nội dung giáo trình), "Buổi vắng" chọn từ buổi thật; **nhiều ảnh + xem trước + bỏ ảnh**; dòng cuối nói rõ ai sẽ xác nhận (tên GV chính / người giao việc) | Bố cục di động theo mockup, token | — |
| **Xác nhận hoàn thành thủ công** | Gộp việc chờ xác nhận + báo cáo trực lớp chờ xác nhận trong 1 danh sách; **Bộ lọc** (tìm, loại, người thực hiện); panel chi tiết: Người thực hiện / Người giao / Ngày giao / Hạn chót / Người xác nhận, nội dung báo cáo + HS cần bổ trợ + link, ghi chú "không ảnh minh chứng", "Cập nhật: … trước"; **Từ chối bắt buộc lý do** (trước có lý do mặc định ghi cứng) | Token, badge "Chờ xác nhận" | — |
| **Bảng KPI tự động** | **Kỳ báo cáo từ tháng → đến tháng** (mockup "Tháng 10/2023 - Tháng 11/2023"); mã nhân sự dùng `employee_code`; **phạm vi**: người có quyền duyệt công việc xem nhân sự (Quản lý cơ sở: chi nhánh mình), người khác **chỉ xem KPI của mình** (trước ai có `work_task.view` cũng xem KPI mọi người) | Nhãn "Chọn nhân sự", token | Cột "Tỷ lệ hoàn thành bài tập" vẫn là số gần đúng (bài nộp chưa gắn bài tập) — như Phase 2 |
| **Dashboard lớp theo ngày / ma trận tuần, TKB + báo cáo phòng / nhu cầu nhân sự** | Đã đối chiếu ở `feat/phase2-mockup-classes` — kiểm tra lại, không có P1 mới | — | — |
| **Dashboard theo vai trò** (BPMN 22 — không có mockup riêng) | **"Hàng chờ cần xử lý"** cho Admin / Quản lý cơ sở (theo chi nhánh): việc chờ xác nhận, báo cáo trực lớp chờ xác nhận, phiếu thu chờ duyệt, ticket đang mở (+ số ticket chưa có người xử lý); Học thuật: số chờ duyệt gồm cả **đề xuất sửa giáo trình**, hàng chờ order đề / kết quả Big Test chờ duyệt — mỗi ô dẫn tới màn xử lý | Token | Dashboard riêng cho Học vụ / Kế toán / Sale: ngoài phạm vi A6/Phase 4 (giữ lưới lối tắt) |
| **Ticket** | Không có mockup trong `ui-full-tinh-nang-menglish` / `roundcuoi-kieulien` → chỉ kiểm tra lại luật "thông báo đúng người" (Phase 4 nền tảng), không đổi giao diện | — | Đối chiếu giao diện ticket: **chưa có mockup** |

**Quyết định phát sinh (tạm, chờ BA):**
- Q8: người xác nhận tính theo **GV chính hiện tại** của lớp lúc duyệt (đổi GV chính sau khi nộp → GV mới xác nhận). GV chính tự nộp báo cáo không ảnh → người giao việc xác nhận. Admin **không** duyệt thay (theo đúng A6) — nếu GV chính nghỉ, Admin đổi GV chính của lớp.
- Không ảnh + lớp chưa có GV chính + không có đầu việc được giao → không cho nộp (không có ai xác nhận).
- Hạn 3 ca khi gắn buổi học: Trước = giờ vào lớp, Trong = giờ tan, Sau = tan + 60 phút; không gắn buổi: 14:00 / 18:00 / 21:30 (giữ mốc cũ).
- "Quá hạn" tính theo ngày + giờ hạn; việc Bị chặn / Chờ xác nhận không chuyển Quá hạn.
- Việc lặp: lượt kế tiếp sinh khi lượt hiện tại **được xác nhận hoàn thành** (không sinh khi Hủy).
**Route mới:** không. **Lệnh mới:** `tasks:mark-overdue` (mỗi 15 phút).
**Migration:** `2026_10_05_110000_add_q8_fields_to_class_reports` (`class_reports.board_images` JSON + chép ảnh cũ, `class_session_id`, `confirmer_id`). Sửa kèm: `ClassReport::$fillable` thiếu `rejection_reason` nên lý do trả về trước đây **không được lưu**.
**Test:** thêm `tests/Feature/Phase4PlatformParityTest.php` (Q8 × 5, TA 3 ca, danh sách công việc, KPI, tài khoản × 2, phân quyền, danh mục, nhật ký, dashboard). Sửa test khẳng định hành vi / nhãn cũ: `UiSweepTest` (nhãn báo cáo trực lớp "Chờ duyệt" → "Chờ xác nhận"), `ActivityLogModuleTest` (tiêu đề "Nhật Ký Vận Hành Toàn Hệ Thống" → "Nhật ký vận hành"), `Phase4PlatformTest` (xuất nhật ký mặc định .xlsx → test CSV gọi `?format=csv`).
**Triển khai:** `php artisan migrate`; cron `schedule:run` phải chạy (lệnh mới `tasks:mark-overdue`); `php artisan storage:link` (ảnh báo cáo trực lớp trên disk `public`); build lại asset (`npm run build`, commit riêng "chore: rebuild assets").

#### Phase 4 — Đối chiếu mockup nhóm Học phí, hóa đơn, thu chi + luật hoàn phí (nhánh `feat/phase4-finance-parity`)
**Luật hoàn phí theo A6 "Hoàn phí" (25/09/2026):**
- [x] **Ưu tiên chuyển nhượng**: form "Tạo yêu cầu xử lý nghỉ giữa khóa" đặt **Chuyển nhượng** đầu tiên (nhãn "Ưu tiên"), Hoàn tiền ghi "Phương án cuối"; chọn Hoàn tiền hiện gợi ý chuyển số buổi dư + nút "Chuyển sang chuyển nhượng" và **bắt buộc "Lý do không chuyển nhượng"** (`tuition_refund_requests.no_transfer_reason`, kiểm tra cả phía server).
- [x] **Chỉ Admin duyệt hoàn tiền** (loại `refund`); Kế toán / Quản lý vẫn có `refund_transfer.approve` cho chuyển nhượng, khất nợ, bảo lưu và từ chối. Người khác thấy "Chờ Admin duyệt".
- [x] **Bắt buộc ảnh bằng chứng** khi Admin duyệt (= chi tiền): chỉ nhận ảnh JPG / PNG / WEBP, kiểm tra theo nội dung qua `SafeUploadService`, lưu **riêng tư** (`storage/app/private/tuition/refund-proofs`), xem qua `tuition.refunds.proof` có kiểm tra phạm vi chi nhánh. Lỗi nghiệp vụ khi duyệt thì xóa file vừa lưu.
- [x] **Hạn xử lý** = min(ngày lập + 7 ngày, cuối tháng lập) cho hoàn tiền và chuyển nhượng (khất nợ / bảo lưu không có hạn). Còn chờ mà quá hạn → cờ **"Quá hạn xử lý · N ngày"**, bộ đếm "N QUÁ HẠN", lọc "Quá hạn xử lý"; đã xử lý sau hạn → "Xử lý trễ hạn". **Không chặn** nút duyệt (thông báo nhắc khi duyệt trễ).
- [x] Giữ lựa chọn **thu hồi hoa hồng** (gợi ý < 1 tháng, sửa được số tiền) trong hộp duyệt hoàn tiền; chuyển nhượng không thu hồi. Từ chối lưu **lý do** (`rejection_reason`).

**Đối chiếu từng màn** (phương pháp như Phase 3: `code.html` của `hoc-phi-va-hoa-don-ui-mockup/*`, `cauhinhhoadon-ui-mockup`, `epic-8-thu-phi-qua-han`, `epic-5/cau-hinh-nhac-no`, `epic-5/cau-hinh-tai-khoan-ngan-hang`, `epic-13-bao-cao-thu-chi/*` cạnh Blade; P1 rồi P2):

| Màn | P1 đã sửa | P2 đã sửa | Chưa làm / bỏ theo A6 (lý do) |
|---|---|---|---|
| **DS thu phí** | Thêm nhóm **Quá hạn** (nghiêm trọng ≥ N / mới 1–N-1 ngày, thẻ học viên + số buổi, "Lập phiếu thu", "Xác nhận đã liên hệ", "Báo cáo Admin", "Gửi nhắc nợ") và **Sắp đến hạn** (14 ngày, phân trang) dùng chung với màn quá hạn; cột **Khoản thu**; thẻ thống kê **theo phạm vi chi nhánh + bộ lọc** (trước cộng cả hệ thống); "Lập phiếu thu" điền sẵn khoản học phí | Tiêu đề / bộ lọc theo mockup, token `x-ui.*`, tiền định dạng VN | "Số buổi nghỉ hè" — chưa có dữ liệu buổi nghỉ hè theo học viên. Giữ thêm bảng "Toàn bộ khoản học phí" (sổ công nợ) dưới cùng |
| **Lập phiếu thu** | Banner **"Học viên vừa chuyển lớp mới"** khi lớp của khoản học phí khác lớp hiện tại; bảng kê liệt kê **từng khoản giáo trình / đồ dùng** (`fee_items`); bỏ ghi chú soạn sẵn và lý do phụ thu tự điền | Page header + badge "Bản nháp" + mã phiếu | "Xuất biên lai" trước khi duyệt không làm (phiếu chưa có số HĐ; in ở Lịch sử thu) |
| **Duyệt phiếu thu** | **Trạng thái đối soát thật** với sao kê SePay theo mã giao dịch (Khớp số tiền & mã giao dịch / Lệch số tiền / Cần đối chiếu thủ công / Tiền mặt); TK thụ hưởng theo hợp đồng → chi nhánh → mặc định; thời gian giao dịch = ngày thu; hàng đợi hiện "Có minh chứng" / số biên lai; **tự làm mới thật** (90 giây, không khi đang mở hộp thoại / gõ); bỏ ghi chú và lý do phụ thu giả; "Đã duyệt hôm nay" theo `approved_at` | Page header, tiền VN, bảng kê "Học phí (Hạn …)" + "Đã miễn giảm" | Mockup "trả về Bản nháp" khi từ chối: giữ "Bị từ chối" (Phase 4 trước). Thông báo cổng phụ huynh khi duyệt: chưa làm |
| **Lịch sử thu** | Lọc tìm kiếm / trạng thái / hình thức / khoảng ngày thu / **theo học viên** (`?student_id`, tiêu đề "Học viên: … • Mã HV"); **"Chỉ khoản thu tái tục"** (bỏ khoản học phí đầu tiên — cùng định nghĩa hoa hồng) kèm banner mockup; cột Khoản thu, Ngày thu, **Đối soát** (Chính thức / Đã hủy HĐ); ngăn **Chi tiết phiếu thu** + minh chứng + "Phê duyệt phiếu" / "Yêu cầu chỉnh sửa"; **Xuất Excel**; "Tải lên biên lai mới"; mẫu in lấy **mẫu số / ký hiệu từ dải số thật** (bỏ `01GTKT0/001`, `C26MEN`, số `HD-…` bịa); phiếu chỉ phụ thu hiện đúng học viên | Token, badge trạng thái, dữ liệu JS qua `@js` (bỏ `addslashes` trong JS) | "In báo cáo" → thay bằng Xuất Excel (không dùng `window.print` cho báo cáo) |
| **Duyệt hủy hóa đơn** | **Chỉ Admin** duyệt / từ chối (mockup "Chỉ Admin phê duyệt"); **Xuất danh sách** (CSV theo bộ lọc); bỏ ảnh / tên file minh chứng giả khi không có ảnh; "Tạo yêu cầu hủy" theo quyền `invoice.request_cancel` | Page header + badge, tiền VN | — |
| **Hoàn tiền & khất nợ** | Bố cục mockup: trái (Đánh dấu khất nợ, Tạo yêu cầu nghỉ giữa khóa: thẻ Học viên / Mã HV / Buổi dư / Đã thu, **tìm học viên nhận + "Chọn"**, "Số dư khả dụng tối đa"), phải **"Yêu cầu chờ phê duyệt"** (quá hạn lên đầu, ✓ / ✕ mở hộp duyệt / từ chối); bảng **Tất cả yêu cầu** có lọc; luật A6 ở trên | Token `x-ui.*` | — |
| **Thu phí quá hạn** | Cột / dòng **Khoản thu**, phân trang nhóm Sắp đến hạn (dùng chung partial với DS thu phí) | — | "Số buổi nghỉ hè" (như trên) |
| **Dải số hóa đơn** | **Phạm vi chi nhánh**: kế toán chi nhánh chỉ thấy / cấp / sửa / ngừng dải của chi nhánh mình, dải mặc định chỉ Admin / kế toán tổng; **thông báo trong app** tới Kế toán + Quản lý cơ sở của chi nhánh khi thêm / sửa / ngừng / dùng lại dải (mockup "Chính sách đồng bộ số hóa đơn") | Ghi chú chính sách | Đồng bộ "phân hệ kế toán" ngoài hệ thống: không có |
| **Tài khoản ngân hàng** | Cột / trường **Loại tài khoản** (Công ty / Khác — cột mới `bank_accounts.account_type`); bảng có **tìm kiếm** + "Hiển thị x trên y"; form Thêm / Sửa bên phải; **ô "Đang sử dụng"** (trước mỗi lần sửa tự bật lại tài khoản), không cho ngừng tài khoản mặc định; khối **Tích hợp VietQR** xem trước QR tài khoản mặc định | Bố cục bảng thay lưới thẻ, token | Tab SePay giữ nguyên |
| **Nhắc nợ (cấu hình)** | Khối thiết lập nhanh theo mockup: **Mốc nhắc trước hạn**, **Mốc nhắc lại (1–3 ngày, báo lỗi đúng câu mockup)**, **Mốc quá hạn bắt buộc liên hệ**, kênh in-app "Hoạt động"; lưu = đồng bộ mốc nhắc thật `NHAC-TRUOC` / `NHAC-LAI` cho lệnh 08:30 (đã có mốc cùng ngày thì bật mốc đó, không tạo trùng) | Mô tả, "Thông tin vận hành" | Zalo ZNS / SMS vẫn "chưa tích hợp" |
| **Báo cáo doanh thu** | **Phạm vi chi nhánh như Học phí** (kế toán chi nhánh chỉ chi nhánh mình — trước thấy toàn hệ thống); doanh thu theo chi nhánh hợp đồng, thiếu thì theo học viên; **Xuất báo cáo theo bộ lọc chi nhánh**; tình trạng dòng tổng tính theo tỷ suất (bỏ "Tăng trưởng" ghi cứng); thêm nhóm **Chi khác**; bỏ ví dụ giả (IELTS Foundation…) và câu sai "cộng phiếu Chờ duyệt / Tạm thu" | Nhãn bỏ "Epic 7", "Chị Liên" | — |
| **Khoản chi vận hành** | Phạm vi chi nhánh như trên (thêm / sửa / xóa chi nhánh khác 403); **Xuất Excel theo cả ô tìm kiếm**; bỏ người lập giả "Admin"; tháng sai định dạng không lỗi 500 | Bỏ chữ kỹ thuật "Epic 7" | — |
| **Nhập học phí Excel** (kèm) | Chỉ nhập vào **chi nhánh trong phạm vi** (danh sách chi nhánh lọc theo phạm vi; tải / xác nhận file chi nhánh khác 403) | — | — |
...
#### RBAC linh hoạt toàn hệ thống (nhánh `feat/rbac-finish`, nối tiếp `feat/rbac-flexible`)
**Mục tiêu (đã duyệt):** Admin cấu hình quyền cho **mọi vai trò (kể cả Học vụ)** và **từng người** mà không sửa code. Thiết kế chi tiết, danh mục quyền, phạm vi dữ liệu theo module, mặc định theo vai trò, cách thêm quyền mới: **`docs/rbac.md`**.

**Đã làm:**
- [x] **Một danh mục quyền duy nhất** `config/permission_catalog.php` (`App\Support\PermissionCatalog`): mọi permission có nhãn + mô tả tiếng Việt, nhóm hiển thị (CRM & Tuyển sinh, Học vụ & Đào tạo, Kế toán / Học phí, Nhân sự – Chấm công – Lương, Vận hành & Hỗ trợ, Hệ thống & Phân quyền, Cổng & Đối tượng). Ba loại: thao tác `module.action`, phạm vi dữ liệu `module.scope_own|scope_branch|scope_all`, quyền đối tượng (`portal.student/teacher/assistant/staff`, `class.teach/assist`, `lead.be_assigned`, `entrance_test.examine`) + quyền động `user.assign_role.<vai trò>`.
- [x] **Bỏ toàn bộ kiểm tra cứng vai trò** trong `app/` và `resources/views` (~95 chỗ: CRM, người dùng, học phí, báo cáo định kỳ, thông báo, hồ sơ học viên, cổng GV / học viên, phạt, dashboard, menu, luồng CRM…) → permission / phạm vi dữ liệu / luật quan hệ. Còn lại **1** chỗ `hasRole` (`User::isSuperAdmin()`) và các chỗ dùng `isSuperAdmin()` trong danh sách ngoại lệ cố định (Gate::before, quản trị phân quyền, miễn trừ Super Admin của luật "người lập phiếu không tự sửa / duyệt", "người vi phạm không tự chốt", tab mặc định màn Công việc, người nhận dự phòng khi ticket chưa có người phân công). Chốt chặn: `tests/Feature/RbacNoHardcodedRolesTest.php`.
- [x] **Phạm vi dữ liệu cấu hình được** (`App\Support\DataScope`) cho 14 module: CRM, học viên, lớp, Big Test, học phí, thu chi, chấm công, lương, KPI, công việc, ticket, nhân sự, nhật ký, dashboard. Thay các scope viết tay (`CrmCustomer`/`Student`/`ClassModel::scopeVisibleTo`, `userManagesAll`, `TuitionBranchScope`, `FinanceController`, `User::managedBranchIds` (đã xóa), công việc, KPI, ticket, dashboard, báo cáo CRM…). `tuition.all_branches` / `finance.all_branches` → `tuition.scope_all` / `finance.scope_all` (migration chuyển cả gán theo vai trò lẫn cá nhân; kế toán tổng cũ nhận thêm `attendance_staff.scope_all` như hành vi cũ).
- [x] **Phân cấp gán vai trò** theo quyền `user.assign_role.<vai trò>` (thay `UserController::creatableRoles`); Super Admin chỉ Super Admin gán. Vai trò mới tự sinh quyền gán, cấp cho vai trò đang gán được mọi vai trò (Quản lý cơ sở) như trước.
- [x] **Màn Vai trò**: ma trận theo module (Xem / Thêm / Sửa / Xóa / Duyệt + Thao tác khác + Phạm vi dữ liệu), tạo / nhân bản / đổi tên hiển thị & mô tả (cột mới `roles.label`, `roles.description`) / đổi mã (vai trò tự tạo) / xóa (chỉ khi chưa gán). **Phân quyền cá nhân**: cùng ma trận, theo vai trò / cấp thêm / thu hồi, phạm vi dữ liệu theo người; giữ "Phạm vi áp dụng" chi nhánh / lớp của Lớp học.
- [x] **An toàn**: Super Admin bất biến; không tự khóa mình (không bỏ được quyền quản trị phân quyền của chính mình, Super Admin không tự bỏ vai trò Super Admin); mọi thay đổi vai trò / quyền / phân quyền cá nhân / gán vai trò ghi nhật ký trước / sau (+ added / removed); xóa cache quyền sau thay đổi.
- [x] **Seed chỉ thêm**: `RoleSeeder` chỉ gán mặc định cho vai trò mới tạo (không ghi đè cấu hình Admin), `PermissionSeeder` chỉ `findOrCreate`; `deploy.sh` không chạy seeder.
- [x] Menu theo quyền: bỏ `roles` trong `SidebarMenu`, nhóm có "quyền neo" (vd. nhóm Học phí cho người xử lý nghiệp vụ kế toán, Cổng giáo viên cho `portal.teacher/assistant`). Route trước đây kiểm tra vai trò trong controller nay có `can:` (báo cáo định kỳ, chấm bài nộp, hoàn tác nhật ký).

**Khác biệt nhỏ có chủ đích so với trước** (mặc định còn lại giữ nguyên, đối chiếu bằng ảnh chụp menu + mã trạng thái của mọi trang GET cho 26 tài khoản demo trước / sau): ô chọn GV chính / GVNN khi tạo lớp liệt kê thêm GV Full-time / Part-time (trước đây bị lọc sót dù được phép gán); ô chọn "Sale phụ trách" khi tạo / sửa khách liệt kê mọi người được nhận phụ trách (Sale, Quản lý, Admin) như luật kiểm tra; người được Admin cấp riêng quyền sửa lớp thấy nhóm menu Lớp học; nút "Order đề" ở màn Chặng đang dạy hiện cho người được làm thay GV (khớp quyền phía server).

**Migration:** `2026_10_07_100000_add_label_and_description_to_roles_table`, `2026_10_07_100100_introduce_flexible_rbac_permissions` (tạo quyền mới, cấp mặc định cho vai trò hệ thống **chỉ với quyền mới**, chuyển `*.all_branches`, suy phạm vi cho vai trò tự tạo / quyền cá nhân như code cũ; cài mới thì chỉ dọn quyền cũ; chạy lại không tạo trùng; có `down`).
- [x] **Hợp nhất `main` (deploy DirectAdmin)**: `ProductionBootstrapSeeder` (hook `/_deploy/hook`, `seed=bootstrap`) dùng seed RBAC chỉ thêm + gán `Rbac::SUPER_ADMIN` cho tài khoản khởi tạo; `DeployHookTest` kiểm tra đủ vai trò / phạm vi, không còn `*.all_branches`. Hook chỉ `set_time_limit` khi chạy qua web (trong CLI làm chết cả bộ test sau 300 giây). Người nhận thông báo là Super Admin dùng hằng `Rbac::SUPER_ADMIN`.
- [x] Migration chuyển `tuition.all_branches` cấp thêm `attendance_staff.scope_all` cho **mọi** nguồn gán (vai trò, quyền trực tiếp, cá nhân) như hành vi cũ.

**Test:** `RbacNoHardcodedRolesTest` (chốt chặn), `RbacFlexibleTest` (mặc định phạm vi theo vai trò; Admin thu hồi quyền của Học vụ → ẩn menu + 403; tạo vai trò tự do → dùng được + phạm vi; nhân bản / đổi tên / xóa; đổi phạm vi own → branch → all đổi danh sách CRM, học viên, học phí; chặn cá nhân thắng quyền vai trò; phân cấp gán vai trò; Super Admin bất biến; không tự khóa; cổng theo đối tượng; seed không ghi đè; mọi quyền có nhãn / mô tả). Cập nhật `BaPermissionsTest` (+ kiểm tra migration chuyển `*.all_branches`) và trait `GrantsPersonalPermissions`. `RbacFlexibleTest::test_upgrade_migration_moves_all_branches_grants_from_roles_direct_and_overrides`. Toàn bộ bộ test xanh: **851 test, 10821 assertion**; `php artisan migrate:fresh --seed` (sqlite) chạy sạch; nâng cấp CSDL seed bằng `main` → `migrate` → `rollback --step=2` → `migrate` kiểm tra thủ công (quyền cũ chuyển đúng, 202 permission).
**Triển khai:** `php artisan migrate --force` (2 migration; migration tự xóa cache quyền — dùng cache ngoài thì chạy thêm `php artisan permission:cache-reset`), rồi `php artisan optimize:clear` như `deploy.sh`. Hook deploy DirectAdmin làm tương tự (`migrate --force` + làm mới cache). **Không** chạy lại seeder trên production (có chạy cũng không ghi đè); cài mới dùng `seed=bootstrap`. Sau deploy: vào **Vai trò** kiểm tra ma trận; kế toán tổng giữ "Phạm vi dữ liệu: Toàn hệ thống" cho Học phí / Thu chi / Chấm công ở Phân quyền cá nhân.
