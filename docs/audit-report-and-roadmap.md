...
| 1 | Chặn rủi ro khẩn + Tuyển sinh → vào lớp | 🟦 | ~90% | Xong: toàn bộ 23 P0, CRM pipeline 8 bước, chốt & xếp lớp, xác nhận chính thức, hồ sơ học viên, bộ sinh mã, chấm test theo khối lớp (Q2), luật lùi bước/Thất bại/học thử, quét dữ liệu giả & nút giả (B2), **đối chiếu 12 màn mockup Phase 1** (xem nhật ký `feat/phase1-mockup-parity`), build assets. Còn: test nghiệm thu trọn luồng, các mục "chưa làm" trong nhật ký đối chiếu |
