...
| 2 | Vận hành lớp học | ✅ | ~97% (phạm vi đã chốt) | Xong: lịch/TKB/dashboard lớp, GVNN, nghỉ lễ (+ xếp bù), trình độ, giáo trình Giáo trình → Chặng → Unit → Buổi (Q4, 1 chặng mở, tự đóng/mở theo Big Test), Big Test (order → **Học thuật duyệt + tự tạo đợt thi** → nhắc 7 ngày (cả theo ngày dự kiến) → kết quả (**nháp / gửi duyệt**) → Học thuật duyệt → Zalo tới **SĐT phụ huynh**; GV chỉ xem phần Speaking), điểm danh theo buổi, bổ trợ, cổng học viên, **chăm sóc tháng đầu 3 mốc A6** (helper cho gate hoa hồng), sinh nhật, portal trợ giảng, KPI board; nghiệm thu trọn luồng + dữ liệu demo; đối chiếu mockup 2 nhóm màn. Quyền duyệt tách riêng (`big_test.approve`, `syllabus.approve_adjustment` chỉ Học thuật + Admin). **Còn lại:** Zalo vận hành thật (OA, template, token), tách phần Speaking trong chính tệp đề (đề chỉ là link ngoài). Chờ BA: xem "Phase 2 — hoàn thiện" và "Nghiệm thu Phase 2" |
