...
<form action="{{ route('tickets.status.update', $ticket->id) }}" method="POST" class="flex items-center gap-1.5">
    @csrf
    <select name="status" onchange="this.form.requestSubmit()" aria-label="Trạng thái ticket" class="text-xs font-bold rounded-xl border border-gray-200 p-2 {{ $ticket->status_badge }}">
