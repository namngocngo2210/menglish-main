        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <nav class="flex items-center gap-1.5 text-gray-400 text-xs mb-1">
                    ...
                    <span class="text-[#F5691A] font-semibold">Gán vai trò chức vụ</span>
                </nav>
            ...
            <div class="lg:col-span-4 bg-white border border-gray-200 rounded-2xl p-6 shadow-sm text-center space-y-4">
                ...
                <div class="border-t border-gray-100 pt-4 text-left text-xs space-y-2.5">
                    ...
                    <div class="flex justify-between py-1 border-b border-gray-50">
                        ...
                        <span class="font-bold text-[#F5691A]">{{ $user->roles->count() }} vai trò</span>
                    </div>
                </div>
            </div>
                ...
                <form method="POST" action="{{ route('users.roles.update', $user) }}" class="space-y-4">
                    ...
                    <div class="space-y-3">
                        @foreach ($roles as $role)
                            <label class="flex items-center justify-between p-4 rounded-2xl border border-gray-200 hover:border-[#F5691A] hover:bg-orange-50/20 cursor-pointer transition shadow-2xs group">
                                <div class="flex items-center gap-3.5">
                                    <input type="checkbox" name="roles[]" value="{{ $role->name }}"
                                        @checked($user->hasRole($role->name))
                                        class="w-4 h-4 rounded border-gray-300 text-[#F5691A] focus:ring-[#F5691A]">
                                    <div>
                                        <div class="font-bold text-sm text-gray-900 group-hover:text-[#F5691A] transition">{{ \App\Helpers\AclHelper::roleLabel($role->name) }}</div>
                    ...
                    <div class="pt-4 border-t border-gray-100 flex items-center justify-end gap-3">
                        ...
                        <button type="submit" class="px-5 py-2.5 rounded-xl bg-[#F5691A] hover:bg-[#d85a15] text-white text-xs font-bold shadow-sm transition flex items-center gap-1.5 cursor-pointer">
