{{-- Trường form "Giao việc mới" (mockup form_giao_vi_c) — dùng chung cho modal ở Danh sách công việc và trang tạo. Biến: $users, $branches, $classes --}}
@php
    $isRecurringOld = old('taskType') === 'recurring';
@endphp
<div class="space-y-md" x-data="{ isRecurring: @js($isRecurringOld) }">
    <x-ui.input name="taskTitle" label="Tiêu đề công việc" required placeholder="Nhập tiêu đề công việc..." maxlength="255" />
    <x-ui.textarea name="taskDescription" label="Mô tả chi tiết" rows="3" placeholder="Mô tả nội dung công việc chi tiết..." />
    <div class="grid grid-cols-1 gap-md sm:grid-cols-2">
        <x-ui.select name="assignee" label="Người nhận" required placeholder="Chọn nhân sự..."
        ...
        <x-ui.input type="date" name="dueDate" label="Hạn hoàn thành" required :value="old('dueDate', now()->addDays(2)->format('Y-m-d'))" />
    </div>
    <div class="grid grid-cols-1 gap-md sm:grid-cols-2">
        <x-ui.select name="branch_id" label="Chi nhánh" placeholder="-- Không chỉ định --" :options="$branches->pluck('name', 'id')" />
        <x-ui.select name="class_id" label="Gắn lớp (nếu có)" placeholder="-- Không gắn lớp --"
    ...
    <fieldset class="space-y-sm rounded-lg border border-outline-variant bg-surface-container-low p-md">
        ...
        <div x-show="isRecurring" x-cloak class="border-t border-outline-variant pt-sm">
            <x-ui.select name="frequency" label="Tần suất" :options="['daily' => 'Hàng ngày', 'weekly' => 'Hàng tuần', 'monthly' => 'Hàng tháng']" :value="old('frequency', 'weekly')"
