...
<x-app-layout title="Tạo lượt giao việc cho Trợ giảng">
    ...
    <div class="mx-auto max-w-4xl"
        ...
        <form action="{{ route('tasks.ta-assign.store') }}" method="POST" class="space-y-lg rounded-xl border border-outline-variant bg-surface-container-lowest p-lg">
            @csrf
            <div class="grid grid-cols-1 gap-md md:grid-cols-3">
                <x-ui.select name="assistant_id" label="Chọn Trợ giảng" required placeholder="-- Chọn Trợ giảng --"
                             :options="$assistants->mapWithKeys(fn ($ta) => [$ta->id => $ta->name])" />
                <x-ui.field label="Ngày giao việc" name="assign_date" for="assign_date" required>
                    <input type="date" id="assign_date" name="assign_date" required x-model="assignDate"
                ...
                <x-ui.select name="branch_id" label="Chi nhánh" placeholder="-- Chọn Chi nhánh --" :options="$branches->pluck('name', 'id')" />
            </div>
