    ...
    <div class="max-w-5xl mx-auto space-y-6">
        <form method="POST" action="{{ $role->exists ? route('roles.update', $role) : route('roles.store') }}" class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach ($permissionsByModule as $module => $permissions)
                        <div class="border border-gray-200 rounded-2xl p-4 bg-gray-50/50 hover:bg-white hover:border-primary transition space-y-3">
            ...
            <div class="pt-4 border-t border-gray-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2.5 rounded-xl bg-primary hover:bg-primary-hover text-white text-xs font-bold shadow-sm transition">
